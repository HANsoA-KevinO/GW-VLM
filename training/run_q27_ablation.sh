#!/usr/bin/env bash
# 第 2 轮融合消融 —— Qwen3.6-27B + patch/自注意力应变编码器,两种粒度(32/64)。
# C 仅图 / B-32 图+应变(32) / B-64 / A-32 仅应变(32) / A-64。自重定向日志。
exec > "$HOME/q27_ablation.log" 2>&1
export HF_HUB_OFFLINE=1 TRANSFORMERS_OFFLINE=1
export PYTORCH_CUDA_ALLOC_CONF=expandable_segments:True
cd "$HOME/GW-VLM" || exit 1
source .venv/bin/activate
CFG=training/configs/fusion_qwen36_27b.yaml
EP=2
run() {   # $1=name $2=use_image $3=use_strain $4=patch_size
  ...
  python -u training/train_fusion.py --config $CFG \
    ...
    --output-dir output/runs/fusion_q27_$1 || { echo "TRAIN_$1_FAIL"; return; }
  echo "===== EVAL $1 $(date +%T) ====="
  python -u evaluation/evaluate_fusion.py --adapter output/runs/fusion_q27_$1 \
...
for n in C B32 B64 A32 A64; do
  f=output/runs/fusion_q27_$n/fusion_eval.json
