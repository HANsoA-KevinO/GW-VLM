...
- **多模态融合(图 ⊕ 原始应变,详见 docs/07)**:原型(Qwen2.5-VL-3B)消融 **C仅图 0.878 > B图+应变 0.795 > A仅应变 0.433(<随机)**。**负结果:小数据(~72事件)下从零训的应变分支学不出、反拖累融合;图是检测主力。前置=注入(E3/E4)把 StrainEncoder 喂饱。** 融合代码:`training/{fusion_model,fusion_collator,train_fusion}.py` + `models/strain_encoder.py` + `evaluation/evaluate_fusion.py`(原生 transformers+PEFT,非 Unsloth)。
