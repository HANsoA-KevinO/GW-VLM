...
> 最后更新：2026-06（参数重构 Part A 验证通过）。
>
> 🧭 **想快速搞清"现在在哪、怎么走到这、所有结果、下一步"→ 先读 [`docs/08_项目全景与现状.md`](docs/08_项目全景与现状.md)(大白话全景,必读)。** 本文件是基建/决策细节;docs/08 是当前状态的权威入口。
...
### E2(多任务)+ 多模态融合 结论
- **E2(检测+参数,Qwen3.6-27B,详见 docs/06)**:检测 ROC-AUC **0.959**(多任务没拖累、略升);参数只有 **distance 高于随机**(响度可读),mass/spin ≈ 随机(模型输出"安全众数 bin")。+ 发现 **Qwen3.6 开 thinking 会把检测打崩**(感知任务推理有害),评估须 `--no-unsloth` + `enable_thinking=False`。
- **多模态融合(图 ⊕ 原始应变,详见 docs/07)**:两轮均**负结果——应变在检测上无增益,图是主力**。
  - 第1轮(CNN编码器 + Qwen2.5-VL-3B,1ep):C仅图 0.878 > B图+应变 0.795 > A仅应变 0.433(<随机)。
  - 第2轮(**patch+自注意力编码器 + Qwen3.6-27B,2ep**):**C仅图 0.9557 ≈ B-32 0.9584 ≈ B-64 0.9560(融合中性,差异在噪声内)**;仅应变 A-32/A-64 = 0.518/0.556(**比CNN的0.43进步、但仍近随机**)。
  - **结论**:换最强模型+更好编码器**没翻盘** → 非模型/编码器问题,是**数据墙(90事件)**;27B纯图已0.96、留给应变空间极小;**应变价值在参数估计(带相位)不在检测,前置=注入E3/E4**。
  - 融合代码:`training/{fusion_model,fusion_collator,train_fusion}.py` + `models/strain_encoder.py`(`StrainPatchEncoder` patch+自注意力)+ `evaluation/evaluate_fusion.py`(原生transformers+PEFT,非Unsloth)。27B需 **fla + causal_conv1d**(本地编译)激活线性注意力快路径,bf16+梯度检查点。

### 下一步候选
1. **注入管线 E3/E4**(最高优先级,多处指向):提弱信号 recall + 喂饱应变/参数估计(需写 `04_inject_signals`/`05`,装 lalsuite/pycbc)。
2. **目标函数改造**:参数估计用序数/回归+区间(替代 bin 上的普通交叉熵,缓解众数塌缩)。
3. 远景:致密 GR 流形 + 偏离/异常检测,驱动理论(见讨论)。
